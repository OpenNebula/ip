...
const downloadURLs = {
    chrome: {
        linux: process_1.env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
            '%s/chromium-browser-snapshots/Linux_x64/%d/%s.zip',
        mac: process_1.env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
            '%s/chromium-browser-snapshots/Mac/%d/%s.zip',
        win32: process_1.env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
            '%s/chromium-browser-snapshots/Win/%d/%s.zip',
        win64: process_1.env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
...
function downloadURL(product, platform, host, revision) {
    const params = [downloadURLs[product][platform],
        host,
        revision];
    if (!process_1.env.PUPPETEER_DOWNLOAD_SCHEMA_URL) {
        params.push(archiveName(product, platform, revision));
    }
    const url = util.format(...params);
    return url;
}
/**
 * @internal
