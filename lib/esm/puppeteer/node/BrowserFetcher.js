...
const downloadURLs = {
    chrome: {
        linux: env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
            '%s/chromium-browser-snapshots/Linux_x64/%d/%s.zip',
        mac: env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
            '%s/chromium-browser-snapshots/Mac/%d/%s.zip',
        win32: env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
            '%s/chromium-browser-snapshots/Win/%d/%s.zip',
        win64: env.PUPPETEER_DOWNLOAD_SCHEMA_URL ||
...
function downloadURL(product, platform, host, revision) {
    const params = [downloadURLs[product][platform],
        host,
        revision];
    if (!env.PUPPETEER_DOWNLOAD_SCHEMA_URL) {
