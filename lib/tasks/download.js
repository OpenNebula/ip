...
const prepend = urlPath => {
  const ext = util.getEnv('CYPRESS_DOWNLOAD_MIRROR_EXT')? `.${util.getEnv('CYPRESS_DOWNLOAD_MIRROR_EXT')}` : ''
  const endpoint = url.resolve(getBaseUrl(), urlPath);
  const platform = os.platform();
  const arch = getRealOsArch();
  return `${endpoint}${ext}?platform=${platform}&arch=${arch}`;
};
...
const getUrl = version => {
  ...
  const folders = util.getEnv('CYPRESS_DOWNLOAD_MIRROR_FOLDERS')? `${util.getEnv('CYPRESS_DOWNLOAD_MIRROR_FOLDERS')}` : 'desktop'
  
  return version ? prepend(`${folders}/${version}`) : prepend(folders)
};
